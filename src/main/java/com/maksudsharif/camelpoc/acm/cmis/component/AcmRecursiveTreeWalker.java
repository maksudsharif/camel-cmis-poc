...
public class AcmRecursiveTreeWalker
{
    private static final Logger LOG = LoggerFactory.getLogger(AcmRecursiveTreeWalker.class);

    private final AcmCMISConsumer cmisConsumer;
    private final boolean readContent;
    private final int readCount;
    private final int pageSize;
    private int totalPolled;

    public AcmRecursiveTreeWalker(AcmCMISConsumer cmisConsumer, boolean readContent, int readCount, int pageSize)
    {
    ...
    int processFolderRecursively(Folder folder) throws Exception
    {
        ...
        while (!finished)
        {
            ...
            {
                if (AcmCMISHelper.isFolder(child))
                {
    ...
    private void processNonFolderNode(CmisObject cmisObject, Folder parentFolder) throws Exception
    {
        ...
        Map<String, Object> properties = AcmCMISHelper.objectProperties(cmisObject);
        properties.put(AcmCamelCMISConstants.CMIS_FOLDER_PATH, parentFolder.getPath());
        if (AcmCMISHelper.isDocument(cmisObject) && readContent)
        {
    ...
    {
        sendNode(AcmCMISHelper.objectProperties(folder), null);
    }
